...
, syncPageId    :: Maybe String
  -- Whether to automatically delete page attachments.
, shouldDeleteAttachments :: Bool
} deriving Show
...
updateContentPage :: ConfluenceConfig -> ConfluenceZipper -> ApiCall Page
updateContentPage config zipper = do
  ...
  attachmentMapping <- syncAttachments config pageId localAttachments remoteAttachments
...
syncAttachments :: ConfluenceConfig -> String -> [ LocalAttachment ] -> [ Attachment ] -> ApiCall [ (LocalAttachment, Attachment) ]
syncAttachments (ConfluenceConfig {shouldDeleteAttachments}) pageId localAttachments remoteAttachments = do
  ...
  if shouldDeleteAttachments
    then forM (Set.toList toRemove) $ (\attachmentName -> do
      ...
      liftIO $ putStrLn "Not deleting attachments."
      forM (Set.toList toRemove) $ (\attachmentName -> liftIO $ (putStrLn $ "Dangling attachment: " ++ attachmentName))
...
sync throttle config path = do
  ...
  result <- runApiCall throttle (confluenceXmlApi config) token $ do
      ...
      -- TODO: update this to take account of renames
      ...
      sequence $ map (updateContentPage config) (filter (notShadowReference . pagePosition . label) (traverseZipper syncZipper))
